...
dm9000_reset(void)
{
    ...
    do {
        rt_kprintf("resetting the DM9000, 1st reset\n");
        delay_ms(50); /* Wait at least 20 us */
	...
	do {
		rt_kprintf("resetting the DM9000, 2nd reset\n");
		delay_ms(50); /* Wait at least 20 us */
	} while (dm9000_io_read(DM9000_NCR) & 1);
	
	/* Check whether the ethernet controller is present */
	if ((dm9000_io_read(DM9000_PIDL) != 0x0) ||
		(dm9000_io_read(DM9000_PIDH) != 0x90))
		rt_kprintf("ERROR: resetting DM9000 -> not responding\n");
}
...
static rt_err_t rt_dm9000_init(rt_device_t dev)
{
	...
	/* Get identfy DM9000, try two times, DM9000 sometimes gets the first read wrong */
	for (i = 0; i < 2; i++)
	{
	...
	if (value != DM9000_ID)
	{
		rt_kprintf("read wrong dm9000 id: 0x%x\n", value);
		return -RT_ERROR;
	}
...
void rt_hw_dm9000_init()
{
	...
	*/
...
#endif
#if 1	
	at91_sys_write(AT91_SMC_SETUP(2), 0xFFFFFFFF);
	at91_sys_write(AT91_SMC_PULSE(2), 0xFFFFFFFF);
	at91_sys_write(AT91_SMC_CYCLE(2), 0xFFFFFFFF);
	at91_sys_write(AT91_SMC_MODE(2), \
	 				AT91_SMC_READMODE |
					AT91_SMC_WRITEMODE |
	 				AT91_SMC_EXNWMODE_DISABLE |
...
#endif
	dm9000_device.dev_addr[0] = 0xde;
	dm9000_device.dev_addr[1] = 0xad;
	dm9000_device.dev_addr[2] = 0xbe;
	dm9000_device.dev_addr[3] = 0xef;
	dm9000_device.dev_addr[4] = 0x01;
	dm9000_device.dev_addr[5] = 0x01;
	...
	/* install interrupt */
	rt_hw_interrupt_install(AT91_PIN_PA24, INTEINT4_7_handler, RT_NULL, "PA24");
	rt_hw_interrupt_umask(AT91_PIN_PA24);
}
